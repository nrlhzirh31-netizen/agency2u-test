<template>
  <div>
    ...
    <v-row class="pa-4" no-gutters>
      <app-section-title :title="$t('label.updateCompany')" class="text-uppercase"></app-section-title>
    </v-row>
    <div class="mx-4">
      <v-form ref="updateForm">
          ...
          <v-expansion-panel class="my-3 elevation-0">
            ...
            <v-expansion-panel-content>
              <v-row class="pb-6" no-gutters>
                ...
                <v-col cols="12" sm="6" md="4" class="pb-3">
                  <div class="pr-15">
                    <app-form-field :label="$t('label.businessRegNo')" v-model.trim="data.companyRegNumber" :placeholder="$t(`label.businessRegNo`)" fieldWidth="100%"></app-form-field>
                  </div>
                </v-col>
                ...
                <v-col cols="12" sm="6" md="4" class="pb-3">
                  <div class="pr-15">
                    <app-form-field
                      ...
                      v-model.trim="data.companyRegion"
                      :items="statesDdl"
        ...
        <v-row class="pa-4" no-gutters>
          <v-col cols="12" class="text-right">
            <app-button v-if="accessEdit" :text="$t('button.update')" :action="updateCompany" color="primary"></app-button>
          </v-col>
        </v-row>
      </v-form>
    </div>
...
import { COMPANY_DETAILS, COMPANY_EDIT } from '@/store/company.module'
...
export default {
  ...
  methods: {
    ...
    getCompany() {
      this.$store.dispatch(`${COMPANY_DETAILS}`, { id: this.searchCriteria.selectedRecordId })
    },
    updateCompany() {
      if (this.$refs.updateForm.validate()) {
        let data = {
          ...
          registrationNumber: this.data.companyRegNumber,
          companyBank: this.data.companyBank,
          companyBankAcc: this.data.companyBankAcc,
          ...
          companyState: this.data.companyRegion,
          companyCountry: this.data.companyCountry
        }

        this.$store.dispatch(`${COMPANY_EDIT}`, { id: this.searchCriteria.selectedRecordId, data })
      }
    }
  },
  beforeDestroy() {}
