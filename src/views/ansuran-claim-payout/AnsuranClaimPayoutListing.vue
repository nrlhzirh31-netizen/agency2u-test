<template>
  <div>
    <v-row class="pa-4" no-gutters>
      <app-section-title :title="$t('label.ansuran') + ' ' + $t('label.claimPayout')" class="font-weight-bold text-uppercase d-inline"></app-section-title>
    </v-row>
    <div class="mx-4">
...
import { ANSURAN_CLAIM_PAYOUT_LIST } from '@/store/ansuran-claim-payout.module'

export default {
  name: 'AnsuranClaimPayoutListing',
  components: {},
  ...
  beforeRouteLeave(to, from, next) {
    if (to.name != ROUTE_NAME.ANSURAN_CLAIM_PAYOUT_DETAILS) {
  ...
  data() {
    return {
      ...
      tableHeaders: [
        ...
        {
          text: localizationHelper.getMessage('label.commissionEarning'),
  ...
  computed: {
    tableRows() {
      return this.$store.state.ansuranClaimPayout.list.data
    }
  },
  watch: {
    tableRows() {
  ...
  methods: {
    initSearchCriteria() {
      ...
      if (searchHelper.hasSearchCriteria(SESSION.ANSURAN_CLAIM_PAYOUT_SEARCH)) {
        isDefault = false
    ...
    getClaimPayoutList(filter) {
      this.$store.dispatch(`${ANSURAN_CLAIM_PAYOUT_LIST}`, { filter })
      this.searchCriteria = filter
      searchHelper.storeSearch(SESSION.ANSURAN_CLAIM_PAYOUT_SEARCH, filter)
    },
    filterByStatus() {
    ...
    toDetailsPage(id) {
      let filter = searchHelper.retrieveSearch(SESSION.ANSURAN_CLAIM_PAYOUT_SEARCH)
      filter.selectedRecordId = id
      console.log('selected record', filter)
      searchHelper.storeSearch(SESSION.ANSURAN_CLAIM_PAYOUT_SEARCH, filter)
