<template>
  <div>
    <v-row class="pa-4" no-gutters>
      <app-section-title :title="$t('label.claimHistory')" class="font-weight-bold text-uppercase d-inline"></app-section-title>
    </v-row>
    <div class="mx-4">
...
import { CLAIM_HISTORY_LIST } from '@/store/claimhistory.module'

export default {
  name: 'ClaimHistoryListing',
  components: {},
  beforeRouteEnter(to, from, next) {
    next((t) => {
      let MODULE_ID = permissionHelper.getAllModules()
      let ACTION = permissionHelper.getAllActions(MODULE_ID.CLAIM_HISTORY)

      t.accessView = permissionHelper.isAuthorizedAction(MODULE_ID.CLAIM_HISTORY, ACTION.READ)
      t.accessEdit = permissionHelper.isAuthorizedAction(MODULE_ID.CLAIM_HISTORY, ACTION.UPDATE)
    })
  },
  beforeRouteLeave(to, from, next) {
    if (to.name != ROUTE_NAME.CLAIM_HISTORY_CREATE && to.name != ROUTE_NAME.CLAIM_HISTORY_DETAILS) {
  ...
  data() {
    return {
      ...
      accessEdit: false,
      searchCriteria: {
        claimHistoryId: SHARED.DEFAULT_ALL_VALUE_STR,
        applicationCode: '',
        nric: '',
        agentCode: '',
        orderNumber: ''
  ...
  computed: {
    tableRows() {
      return this.$store.state.claimhistory.list.data
    }
  },
  watch: {
    tableRows() {
  ...
  methods: {
    initSearchCriteria() {
      ...
      if (searchHelper.hasSearchCriteria(SESSION.CLAIM_HISTORY_SEARCH)) {
        isDefault = false
      ...
      this.getClaimHistoryList(this.searchCriteria)
    },
    searchListing() {
    ...
    getClaimHistoryList(filter) {
      this.$store.dispatch(`${CLAIM_HISTORY_LIST}`, { filter })
    ...
    resetSearchCriteria() {
      this.searchCriteria.claimHistoryId = SHARED.DEFAULT_ALL_VALUE_STR
