<template>
  <div>
    ...
    <v-row class="pa-4" no-gutters>
      <app-section-title :title="$t('label.updateApplicationType')" class="text-uppercase"></app-section-title>
    </v-row>
    <div class="mx-4">
            ...
            <v-expansion-panel-content>
              ...
              <v-row class="py-4" no-gutters>
                ...
                <v-col cols="12" class="text-right">
                  <app-button v-if="accessEdit" :text="$t('button.update')" :action="updateApplicationType" color="primary"></app-button>
                </v-col>
              </v-row>
            </v-expansion-panel-content>
          </v-expansion-panel>
...
import { APPLICATION_TYPE_DETAILS, APPLICATION_TYPE_EDIT } from '@/store/applicationtype.module'

export default {
  name: 'ApplicationTypeDetails',
  components: {},
  beforeRouteEnter(to, from, next) {
    next((t) => {
      let moduleIds = permissionHelper.getAllModules()
      let actions = permissionHelper.getAllActions(moduleIds.applicationTypes)

      t.accessEdit = permissionHelper.isAuthorizedAction(moduleIds.applicationTypes, actions.update)
    })
  },
  ...
  computed: {
    data() {
      return this.$store.state.applicationtype.details
    }
  },
  watch: {},
  methods: {
    getApplicationType() {
      this.$store.dispatch(`${APPLICATION_TYPE_DETAILS}`, { id: this.searchCriteria.selectedRecordId })
    },
    updateApplicationType() {
      if (this.$refs.updateForm.validate()) {
        ...
        this.$store.dispatch(`${APPLICATION_TYPE_EDIT}`, { id: this.searchCriteria.selectedRecordId, data })
      }
    }
  },
  beforeDestroy() {}
