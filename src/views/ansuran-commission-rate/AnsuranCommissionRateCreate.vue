<template>
  <div>
    ...
    <v-row class="pa-4" no-gutters>
      <app-section-title :title="$t('label.ai') + ' ' + $t('label.createCommissionRate')" class="text-uppercase"></app-section-title>
    </v-row>
    <div class="mx-4">
        ...
        <v-expansion-panels v-model="openPanelIndex" multiple>
          ...
          <v-expansion-panel class="my-3 elevation-0">
            <v-expansion-panel-header>
              <div class="px-2">
                <app-section-title :title="$t('label.ai') + ' ' + $t('label.commissionRateDetails')" class="text-uppercase"></app-section-title>
              </div>
            </v-expansion-panel-header>
            <v-expansion-panel-content>
              ...
              <v-row class="pb-4" no-gutters>
                ...
                <v-col cols="12" class="text-right">
                  <app-button v-if="accessCreate" :text="$t('button.create')" :action="createAnsuranCommissionRate" color="primary"></app-button>
                </v-col>
              </v-row>
            </v-expansion-panel-content>
          </v-expansion-panel>
...
import { ANSURAN_COMMISSION_RATE_CREATE } from '@/store/ansuran-commission-rate.module'
...
export default {
  name: 'AnsuranCommissionRateCreate',
  components: {},
  beforeRouteEnter(to, from, next) {
    next((t) => {
      ...
      t.accessCreate = permissionHelper.isAuthorizedAction(MODULE_ID.ANSURAN_COMMISSION_RATE, ACTION.CREATE)
    })
  },
  ...
  data() {
    return {
      ...
      data: new AnsuranCommissionRate(),
      openPanelIndex: [0]
    }
  },
  created() {
  ...
  methods: {
    ...
    createAnsuranCommissionRate() {
      if (this.$refs.createForm.validate()) {
        ...
        this.$store.dispatch(`${ANSURAN_COMMISSION_RATE_CREATE}`, { data })
      }
    }
  },
  beforeDestroy() {}
