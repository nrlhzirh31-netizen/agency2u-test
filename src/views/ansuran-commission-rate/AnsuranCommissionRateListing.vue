<template>
  <div>
    <v-row class="pa-4" no-gutters>
      <app-section-title :title="$t('label.ansuran') + ' ' + $t('label.commissionRate')" class="font-weight-bold text-uppercase d-inline"></app-section-title>
    </v-row>
    <div class="mx-4">
          ...
          <v-data-table :headers="tableHeaders" :items="tableRowsFiltered" :items-per-page="pageSize" :no-data-text="$t('message.noRecordFound')" class="app-table-card" disable-sort>
            <template v-slot:item="i">
              <tr>
                ...
                <td class="text-caption font-weight-medium text-right">{{ i.item.fromAmount | moneyFormat }}</td>
                <td class="text-caption font-weight-medium text-right">{{ i.item.toAmount | moneyFormat }}</td>
                <td class="text-caption font-weight-medium text-right">{{ i.item.commissionAmount | moneyFormat }}</td>
...
import { ANSURAN_COMMISSION_RATE_LIST, ANSURAN_COMMISSION_RATE_DELETE } from '@/store/ansuran-commission-rate.module'
...
export default {
  name: 'AnsuranCommissionRateListing',
  components: {},
  ...
  beforeRouteLeave(to, from, next) {
    if (to.name != ROUTE_NAME.ANSURAN_COMMISSION_RATE_CREATE && to.name != ROUTE_NAME.ANSURAN_COMMISSION_RATE_DETAILS) {
  ...
  data() {
    return {
      ...
      tableHeaders: [
        ...
        {
          text: localizationHelper.getMessage('label.fromAmount'),
          class: 'caption',
          align: 'right'
        },
        {
          text: localizationHelper.getMessage('label.toAmount'),
          class: 'caption',
          align: 'right'
        },
        {
          text: localizationHelper.getMessage('label.commissionAmount'),
          class: 'caption',
          align: 'right'
        },
        {
  ...
  computed: {
    tableRows() {
      return this.$store.state.ansuranCommissionRate.list.data
    },
    roleDdl() {
  ...
  methods: {
    ...
    initSearchCriteria() {
      ...
      if (searchHelper.hasSearchCriteria(SESSION.ANSURAN_COMMISSION_RATE_SEARCH)) {
        isDefault = false
      ...
      this.getAnsuranCommissionRateList(this.searchCriteria)
    },
    searchListing() {
    ...
    getAnsuranCommissionRateList(filter) {
      let param = {
        role: filter.role
      }
      this.$store.dispatch(`${ANSURAN_COMMISSION_RATE_LIST}`, { filter: param })
      this.searchCriteria = filter
      searchHelper.storeSearch(SESSION.ANSURAN_COMMISSION_RATE_SEARCH, filter)
    },
    filterByStatus() {
    ...
    toDetailsPage(id) {
      let filter = searchHelper.retrieveSearch(SESSION.ANSURAN_COMMISSION_RATE_SEARCH)
    ...
    deleteItem(id) {
      this.$store.dispatch(ANSURAN_COMMISSION_RATE_DELETE, { id })
    },
