<template>
  <div>
    ...
    <v-row class="pa-4" no-gutters>
      <app-section-title :title="$t('label.ansuran') + ' ' + $t('label.claimHistory')" class="text-uppercase"></app-section-title>
    </v-row>
    <div class="mx-4">
...
import { searchHelper, localizationHelper, ddlHelper } from '@/utils'
import { ANSURAN_CLAIM_HISTORY_DETAILS } from '@/store/ansuran-claim-history.module'
import { LOOKUP_ANSURAN_STATUS_DDL, LOOKUP_PCO_CATEGORY_DDL, LOOKUP_ANSURAN_APPLICATION_FIRM_DDL } from '@/store/lookup.module'
import { moneyFormat } from '@/utils/filters.helper'

export default {
  name: 'AnsuranClaimHistoryDetails',
  components: {},
  beforeRouteLeave(to, from, next) {
    if (to.name != ROUTE_NAME.ANSURAN_CLAIM_HISTORY_LIST) {
      searchHelper.clearSearch(SESSION.ANSURAN_CLAIM_HISTORY_SEARCH)
      searchHelper.clearSearch(SESSION.ANSURAN_CLAIM_HISTORY_DETAILS_SEARCH)
    }
    next()
  },
  props: {},
  ...
  computed: {
    data() {
      return this.$store.state.ansuranClaimHistory.details
    },
    ansuranStatusDdl() {
  ...
  methods: {
    getAnsuranClaimHistory() {
      this.$store.dispatch(`${ANSURAN_CLAIM_HISTORY_DETAILS}`, { id: this.searchCriteria.selectedRecordId, applicationId: this.searchCriteria.selectedRecordId })
    },
    getAnsuranStatusDdl() {
