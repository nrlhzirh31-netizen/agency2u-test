<template>
  <div>
    ...
    <v-row class="pa-4" no-gutters>
      <app-section-title :title="$t('label.updateBank')" class="text-uppercase"></app-section-title>
    </v-row>
    <div class="mx-4">
            ...
            <v-expansion-panel-content>
              ...
              <v-row class="py-4" no-gutters>
                ...
                <v-col cols="12" class="text-right">
                  <app-button v-if="accessEdit" :text="$t('button.update')" :action="updateBank" color="primary"></app-button>
                </v-col>
              </v-row>
            </v-expansion-panel-content>
          </v-expansion-panel>
...
import { BANK_DETAILS, BANK_EDIT } from '@/store/bank.module'
...
export default {
  ...
  computed: {
    data() {
      return this.$store.state.bank.details
    }
  },
  watch: {},
  methods: {
    getBank() {
      this.$store.dispatch(`${BANK_DETAILS}`, { id: this.searchCriteria.selectedRecordId })
    },
    updateBank() {
      if (this.$refs.updateForm.validate()) {
        ...
        this.$store.dispatch(`${BANK_EDIT}`, { id: this.searchCriteria.selectedRecordId, data })
      }
    }
  },
  beforeDestroy() {}
