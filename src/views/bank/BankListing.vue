    ...
    <app-section-card>
      <v-row class="pa-4" no-gutters>
        <v-col cols="auto" class="text-left">
          <app-section-title :title="$t('label.bank')" class="font-weight-bold text-uppercase d-inline"></app-section-title>
        </v-col>
        <v-spacer></v-spacer>
        <!-- Search Criteria -->
        <v-col cols="auto" class="text-right">
          <v-form ref="searchForm" class="search-form-field-set d-inline-block">
            <app-search-form-field :placeholder="$t('label.bankId')" v-model.trim="searchCriteria.bankId"></app-search-form-field>
...
import { BANK_LIST, BANK_DELETE } from '@/store/bank.module'
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    next((t) => {
      ...
      t.accessView = permissionHelper.isAuthorizedAction(MODULE_ID.COMMON_PARAMS, ACTION.READ)
      t.accessCreate = permissionHelper.isAuthorizedAction(MODULE_ID.COMMON_PARAMS, ACTION.CREATE)
      t.accessEdit = permissionHelper.isAuthorizedAction(MODULE_ID.COMMON_PARAMS, ACTION.UPDATE)
      t.accessDelete = permissionHelper.isAuthorizedAction(MODULE_ID.COMMON_PARAMS, ACTION.DELETE)
    })
  },
  beforeRouteLeave(to, from, next) {
    if (to.name != ROUTE_NAME.BANK_CREATE && to.name != ROUTE_NAME.BANK_DETAILS) {
  ...
  computed: {
    tableRows() {
      return this.$store.state.bank.list.data
    }
  },
  watch: {
    tableRows() {
  ...
  methods: {
    initSearchCriteria() {
      ...
      if (searchHelper.hasSearchCriteria(SESSION.BANK_SEARCH)) {
        isDefault = false
        this.searchCriteria = searchHelper.retrieveSearch(SESSION.BANK_SEARCH)
      }
    ...
    getBankList(filter) {
      this.$store.dispatch(`${BANK_LIST}`, { filter })
      this.searchCriteria = filter
      searchHelper.storeSearch(SESSION.BANK_SEARCH, filter)
    },
    filterByStatus() {
    ...
    toDetailsPage(id) {
      let filter = searchHelper.retrieveSearch(SESSION.BANK_SEARCH)
    ...
    deleteItem(id) {
      this.$store.dispatch(BANK_DELETE, { id })
    },
