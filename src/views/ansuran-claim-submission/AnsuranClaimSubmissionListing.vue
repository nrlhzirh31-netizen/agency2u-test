<template>
  <div>
    <v-row class="pa-4" no-gutters>
      <app-section-title :title="$t('label.ansuran') + ' ' + $t('label.claimSubmission')" class="font-weight-bold text-uppercase d-inline"></app-section-title>
    </v-row>
    <div class="mx-4">
...
import { ANSURAN_CLAIM_SUBMISSION_LIST, ANSURAN_CLAIM_SUBMISSION_EDIT } from '@/store/ansuran-claim-submission.module'
import { SHARED_SHOW_DIALOG, SHARED_CLOSE_DIALOG } from '@/store/shared.module'

export default {
  name: 'AnsuranClaimSubmissionListing',
  components: {},
  beforeRouteEnter(to, from, next) {
    next((t) => {
      ...
      t.accessView = permissionHelper.isAuthorizedAction(MODULE_ID.ANSURAN_CLAIM_SUBMISSION, ACTION.READ)
      t.accessEdit = permissionHelper.isAuthorizedAction(MODULE_ID.ANSURAN_CLAIM_SUBMISSION, ACTION.UPDATE)
    })
  },
  beforeRouteLeave(to, from, next) {
    if (to.name != ROUTE_NAME.ANSURAN_CLAIM_SUBMISSION_CREATE && to.name != ROUTE_NAME.ANSURAN_CLAIM_SUBMISSION_DETAILS) {
  ...
  data() {
    return {
      ...
      accessEdit: false,
      searchCriteria: {
        ansuranClaimSubmissionId: SHARED.DEFAULT_ALL_VALUE_STR,
        code: '',
        nric: '',
        agentCode: ''
      },
  ...
  computed: {
    tableRows() {
      return this.$store.state.ansuranClaimSubmission.list.data
    },
    hasMessageDialog() {
      return this.$store.state.shared.messageDialogDisplay.messageTitle
    }
  },
  watch: {
    tableRows() {
  ...
  methods: {
    initSearchCriteria() {
      ...
      if (searchHelper.hasSearchCriteria(SESSION.ANSURAN_CLAIM_SUBMISSION_SEARCH)) {
        isDefault = false
      ...
      this.getAnsuranClaimSubmissionList(this.searchCriteria)
    },
    searchListing() {
    ...
    getAnsuranClaimSubmissionList(filter) {
      this.$store.dispatch(`${ANSURAN_CLAIM_SUBMISSION_LIST}`, { filter })
      this.searchCriteria = filter
      searchHelper.storeSearch(SESSION.ANSURAN_CLAIM_SUBMISSION_SEARCH, filter)
    },
    filterByStatus() {
      this.tableRowsFiltered = this.tableRows
    },
    resetSearchCriteria() {
      this.searchCriteria.ansuranClaimSubmissionId = SHARED.DEFAULT_ALL_VALUE_STR
    ...
    toDetailsPage(id) {
      let filter = searchHelper.retrieveSearch(SESSION.ANSURAN_CLAIM_SUBMISSION_SEARCH)
    ...
    async updateClaim() {
      ...
      if (d != '' && d != 'undefined' && d != null) {
        d.map((item) => {
          let r = {
            payoutId: item.ansuranPayouts[0].id, // assume only 1 payout
        ...
        await this.$store.dispatch(`${ANSURAN_CLAIM_SUBMISSION_EDIT}`, { data })
      } else {
        let buttonAction = () => {
