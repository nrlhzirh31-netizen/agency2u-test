<template>
  <div>
    <v-row class="pa-4" no-gutters>
      <app-section-title :title="$t('label.ansuran') + ' ' + $t('label.reportSalesTransaction')" class="font-weight-bold text-uppercase d-inline"></app-section-title>
    </v-row>
    <div class="mx-4">
              ...
              <v-data-table :headers="tableHeaders" :items="tableRowsFiltered" :items-per-page="pageSize" :no-data-text="$t('message.noRecordFound')" class="app-table-card" disable-sort>
                <template v-slot:item="i">
                  <tr>
                    ...
                    <td class="text-caption font-weight-medium">{{ i.item.transactionDate }}</td>
                    <td class="text-caption font-weight-medium">{{ i.item.invoiceNumber }}</td>
                    <td class="text-caption font-weight-medium">{{ i.item.ansuranStatus }}</td>
                    <td class="text-caption font-weight-medium">{{ i.item.productModel }}</td>
                    <td class="text-caption font-weight-medium">{{ i.item.creditPrice }}</td>
                    <td class="text-caption font-weight-medium">{{ i.item.pco }}</td>
                  </tr>
                </template>
              </v-data-table>
            </v-col>
...
import { ANSURAN_REPORT_SALES_TRANSACTION } from '@/store/ansuran-report.module'

export default {
  name: 'AnsuranReportSalesTransaction',
  components: {},
  beforeRouteEnter(to, from, next) {
    next((t) => {
      let MODULE_ID = permissionHelper.getAllModules()
      let ACTION = permissionHelper.getAllActions(MODULE_ID.ANSURAN_REPORT)

      t.accessView = permissionHelper.isAuthorizedAction(MODULE_ID.ANSURAN_REPORT, ACTION.READ)
      t.accessCreate = permissionHelper.isAuthorizedAction(MODULE_ID.ANSURAN_REPORT, ACTION.CREATE)
      t.accessEdit = permissionHelper.isAuthorizedAction(MODULE_ID.ANSURAN_REPORT, ACTION.UPDATE)
      t.accessDelete = permissionHelper.isAuthorizedAction(MODULE_ID.ANSURAN_REPORT, ACTION.DELETE)
    })
  },
  beforeRouteLeave(to, from, next) {
    ...
    // }
    searchHelper.clearSearch(SESSION.ANSURAN_REPORT_SALES_TRANSACTION)
    next()
  },
  props: {},
  data() {
    return {
      ...
      tableHeaders: [
        ...
        {
          text: localizationHelper.getMessage('label.invoiceNumber'),
          class: 'caption'
        },
        {
        ...
        {
          text: localizationHelper.getMessage('label.productModel'),
          class: 'caption'
        },
        {
  ...
  computed: {
    tableRows() {
      return this.$store.state.ansuranReport.ansuranSaleslist.data
    }
  },
  watch: {
    tableRows() {
  ...
  methods: {
    initSearchCriteria() {
      ...
      if (searchHelper.hasSearchCriteria(SESSION.ANSURAN_REPORT_SALES_TRANSACTION)) {
        isDefault = false
        this.searchCriteria = searchHelper.retrieveSearch(SESSION.ANSURAN_REPORT_SALES_TRANSACTION)
        this.getAnsuranReportSalesTransaction(this.searchCriteria)
      }
    ...
    getAnsuranReportSalesTransaction(filter) {
      let param = {
        fromDate: filter.fromDate,
        endDate: filter.endDate
      }
      this.$store.dispatch(`${ANSURAN_REPORT_SALES_TRANSACTION}`, { filter: param })
      this.searchCriteria = filter
      searchHelper.storeSearch(SESSION.ANSURAN_REPORT_SALES_TRANSACTION, filter)
    },
    filterByStatus() {
