...
import { APPLICATION_HISTORY } from '@/store/customerapplication.module'
...
export default {
  ...
  beforeRouteLeave(to, from, next) {
    if (to.name != ROUTE_NAME.APPLICATION_HISTORY_DETAILS) {
  ...
  computed: {
    tableRows() {
      return this.$store.state.customerapplication.history.data
    },
  ...
  watch: {
    ...
    applicationStatusDdl() {
      this.searchCriteria.applicationStatus = 'COMPLETED'
    },
    applicationTypeDdl() {
  ...
  methods: {
    ...
    initSearchCriteria() {
      ...
      if (searchHelper.hasSearchCriteria(SESSION.APPLICATION_HISTORY_SEARCH)) {
        isDefault = false
    ...
    getCustomerApplicationList(filter) {
      ...
      this.$store.dispatch(`${APPLICATION_HISTORY}`, { filter: param })
      this.searchCriteria = filter
      searchHelper.storeSearch(SESSION.APPLICATION_HISTORY_SEARCH, filter)
    },
    filterByStatus() {
    ...
    toDetailsPage(id) {
      let filter = searchHelper.retrieveSearch(SESSION.APPLICATION_HISTORY_SEARCH)
