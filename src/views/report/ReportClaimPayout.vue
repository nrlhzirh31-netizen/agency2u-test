<template>
  <div>
    <v-row class="pa-4" no-gutters>
      <app-section-title :title="$t('label.reportClaimPayout')" class="font-weight-bold text-uppercase d-inline"></app-section-title>
    </v-row>
    <div class="mx-4">
...
export default {
  ...
  data() {
    return {
      ...
      tableHeaders: [
        ...
        {
          text: localizationHelper.getMessage('label.totalAmountClaim'),
          class: 'caption'
        }
      ],
  ...
  methods: {
    ...
    initSearchCriteria() {
      ...
      if (searchHelper.hasSearchCriteria(SESSION.REPORT_CLAIM_PAYOUT)) {
        isDefault = false
        this.searchCriteria = searchHelper.retrieveSearch(SESSION.REPORT_CLAIM_PAYOUT)
        this.getReportClaimPayout(this.searchCriteria)
      }
    ...
    getReportClaimPayout(filter) {
      let param = {
        user: filter.user,
        fromDate: filter.fromDate,
        endDate: filter.endDate
      }
      this.$store.dispatch(`${REPORT_PAYOUT}`, { filter: param })
      this.searchCriteria = filter
      searchHelper.storeSearch(SESSION.REPORT_CLAIM_PAYOUT, filter)
    },
    filterByStatus() {
