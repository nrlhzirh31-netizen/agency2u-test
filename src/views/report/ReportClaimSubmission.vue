              ...
              <v-data-table :headers="tableHeaders" :items="tableRowsFiltered" :items-per-page="pageSize" :no-data-text="$t('message.noRecordFound')" class="app-table-card" disable-sort>
                <template v-slot:item="i">
                  <tr>
                    ...
                    <td class="text-caption font-weight-medium">{{ i.item.commission }}</td>
...
import { REPORT_SUBMISSION } from '@/store/report.module'
import { LOOKUP_USERCODE_DDL } from '@/store/lookup.module'

export default {
  name: 'ReportClaimSubmission',
  components: {},
  ...
  beforeRouteLeave(to, from, next) {
    ...
    // }
    searchHelper.clearSearch(SESSION.REPORT_CLAIM_SUBMISSION)
    next()
  },
  props: {},
  data() {
    return {
      ...
      tableHeaders: [
        ...
        {
          text: localizationHelper.getMessage('label.commission'),
          class: 'caption'
        },
        {
  ...
  computed: {
    tableRows() {
      return this.$store.state.report.submmissionlist.data
    },
    userDdl() {
  ...
  methods: {
    ...
    initSearchCriteria() {
      ...
      if (searchHelper.hasSearchCriteria(SESSION.REPORT_CLAIM_SUBMISSION)) {
        isDefault = false
        this.searchCriteria = searchHelper.retrieveSearch(SESSION.REPORT_CLAIM_SUBMISSION)
        this.getReportClaimSubmission(this.searchCriteria)
      }
    ...
    getReportClaimSubmission(filter) {
      let param = {
        user: filter.user,
      ...
      this.$store.dispatch(`${REPORT_SUBMISSION}`, { filter: param })
      this.searchCriteria = filter
      searchHelper.storeSearch(SESSION.REPORT_CLAIM_SUBMISSION, filter)
    },
    filterByStatus() {
